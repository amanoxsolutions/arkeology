...
# ---------------------------------------------------------------------------
# Test 11: an unexpected internal error must not leak raw exception text
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_artifact_resource_internal_error_does_not_leak_exception_text(
    settings: Settings,
    s3_client: S3ClientImpl,
    vectors_client_8: VectorsClientImpl,
    bedrock_8: FakeBedrockClient,
    ...
    """An unexpected exception surfacing from the arkeology://artifact/{id*} handler
    must be logged server-side but never echoed to the MCP caller — the raw text can
    carry AWS-specific details (ARNs, account IDs, bucket names)."""
    import arkeology.resources as resources_module

    mocker.patch.object(
        resources_module,
        "_read_artifact",
        side_effect=RuntimeError("arn:aws:s3:::secret-bucket/leak"),
    )

    app = fastmcp.FastMCP(name="test-arkeology-leak", version="0.0.0")
    register_data_resources(app, settings, s3_client, vectors_client_8, bedrock_8)

    read_result = await app.read_resource("arkeology://artifact/some-id")
...
@pytest.mark.asyncio
async def test_artifacts_resource_internal_error_does_not_leak_exception_text(
    settings: Settings,
    s3_client: S3ClientImpl,
    vectors_client_8: VectorsClientImpl,
    bedrock_8: FakeBedrockClient,
    ...
    """An unexpected exception surfacing from the arkeology://artifacts handler must be
    logged server-side but never echoed to the MCP caller."""
    import arkeology.resources as resources_module
    ...
    app = fastmcp.FastMCP(name="test-arkeology-leak-listing", version="0.0.0")
    register_data_resources(app, settings, s3_client, vectors_client_8, bedrock_8)

    read_result = await app.read_resource("arkeology://artifacts")
