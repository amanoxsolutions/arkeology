"""Direct unit tests for arkeology.tools._concurrency.clamp_concurrency.

Task 68 (Phase 13, codebase-hygiene batch 2): clamp_concurrency is a newly
...
from arkeology.tools._concurrency import clamp_concurrency


def test_value_within_range_passes_through_with_no_warning() -> None:
    effective, warning = clamp_concurrency(5, default=3, max_=15)
    assert effective == 5
    assert warning is None


def test_value_above_max_is_capped_with_warning() -> None:
    effective, warning = clamp_concurrency(20, default=3, max_=15)
    assert effective == 15
    assert warning == (
...
def test_value_below_one_is_substituted_with_default_and_warning() -> None:
    effective, warning = clamp_concurrency(0, default=3, max_=15)
    assert effective == 3
    assert warning == (
...
def test_value_exactly_at_max_passes_through_with_no_warning() -> None:
    effective, warning = clamp_concurrency(15, default=3, max_=15)
    assert effective == 15
    assert warning is None


def test_value_exactly_one_passes_through_with_no_warning() -> None:
    effective, warning = clamp_concurrency(1, default=3, max_=15)
    assert effective == 1
    assert warning is None
