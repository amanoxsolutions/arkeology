"""Configuration model for arkeology.
...
class Settings(BaseSettings):
    """All configuration for the arkeology server, sourced from environment variables."""
    ...
    FAILURE_LOG_PATH: Annotated[
        str,
        Field(
            default=".arkeology_failures.jsonl",
            description=(
                "Path to the local partial-write failure log (.jsonl). "
                "Defaults to '.arkeology_failures.jsonl' in the working directory."
            ),
        ),
    ]
...
def load_settings(**kwargs: Any) -> Settings:
    ...
    Callers only need to handle :class:`~arkeology.errors.ConfigurationError`
    ...
    try:
        ...
        raise ConfigurationError(
            "Configuration error — fix the following before starting arkeology:\n"
            + "\n".join(messages),
            fields=fields,
        ) from exc
