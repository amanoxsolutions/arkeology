"""cairn_mcp.tools._section_pipeline — shared write-path section embedding pipeline.
...
class PreparedSection:
    ...
    Attributes:
        heading: Original section heading — callers derive the vector key suffix
            from this via ``cairn_mcp.artifact.section_slug``.
...
def prepare_sections_for_embedding(
    ...
    *,
    ...
    1. Parse ``##`` sections from ``content`` (:func:`cairn_mcp.artifact.parse_sections`).
