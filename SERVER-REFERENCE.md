...
| `reconcile_index` | Replay the failure log and scan for orphaned S3 objects, re-indexing any artifacts present in S3 but absent from the vector index | — | `reconciled` (list of re-indexed IDs with section counts), `failed` (list of IDs that failed again), `stuck_failures` (present only when non-empty — entries that have now failed 3 times in a row and are no longer auto-retried; needs manual investigation), `orphans_found`, `total_reconciled`, `failure_log_entries_before` / `after` |
