...
| `reconcile_index` | Replay the failure log and scan for orphaned S3 objects, re-indexing any artifacts present in S3 but absent from the vector index; also directly deletes orphan vector keys logged by `write_artifact`'s Step 8 cleanup when its inline retry is exhausted | — | `reconciled` (list of re-indexed IDs with section counts, or — for orphan-vector-cleanup entries — `orphan_keys_deleted` count with `source: "orphan_vector_cleanup"` in place of section counts), `failed` (list of IDs that failed again), `stuck_failures` (present only when non-empty — entries that have now failed 3 times in a row and are no longer auto-retried; needs manual investigation); `failed`/`stuck_failures` entries for orphan-vector-cleanup failures also carry an `orphan_keys` field, `orphans_found`, `total_reconciled`, `failure_log_entries_before` / `after` |
