...
| `list_artifacts` | List artifact metadata with optional filters; defaults to active artifacts | optional: `type`, `team`, `project`, `tier`, `status` (pass `"all"` to return artifacts regardless of status), `tags`, `commit_refs` (matched against the vector index's derived filter copy, which holds only an artifact's most-recently-appended 20 commit refs — an artifact linked to more is not matched on its oldest SHAs, though the `commit_refs` it returns is the full annotation-backed list) | List of artifact metadata records |
...
| `health_check` | Per-component connectivity status (S3, vectors, Bedrock, write prefix, the annotation store, read prefixes) | — | Status dict; never raises |
...
| `reconcile_index` | Replay the failure log and scan for orphaned S3 objects, re-indexing any artifacts present in S3 but absent from the vector index; also directly deletes orphan vector keys logged by `write_artifact`'s Step 8 cleanup when its inline retry is exhausted | — | `reconciled` (list of re-indexed IDs with section counts, or — for orphan-vector-cleanup entries — `orphan_keys_deleted` count with `source: "orphan_vector_cleanup"` in place of section counts; an entry whose artifact has since been deleted appears once with `source: "failure_log_obsolete"` and no section count, then is dropped from the log), `failed` (list of IDs that failed again), `stuck_failures` (present only when non-empty — entries that have now failed 3 times in a row and are no longer auto-retried; needs manual investigation); `failed`/`stuck_failures` entries for orphan-vector-cleanup failures also carry an `orphan_keys` field, `skipped_non_artifacts` (present only when non-empty — own-scope keys the orphan scan found without vectors but declined to index, because their S3 metadata carries no recognised artifact `type`), `orphans_found`, `total_reconciled`, `failure_log_entries_before` / `after` |
...
| `link_metadata` | Backfill `commit_refs` and/or `references` onto confirmed own-scope artifacts — writes the full, uncapped values to the durable S3 annotation copy; `commit_refs` is also written to vector metadata (capped to the most-recently-appended 20 entries), `references` is never written to vector metadata; no re-embedding | `artifact_ids`, optional `commit_refs`, optional `references` | `linked`, `skipped`, `next_since_ulid` |
