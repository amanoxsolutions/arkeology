...
# dependencies = ["pyyaml"]
# ///
"""Validate ARKEOLOGY_IMPORT.yaml before executing migration (Step 3.B5).
...
def main() -> int:
    manifest_path = Path("ARKEOLOGY_IMPORT.yaml")
    if not manifest_path.exists():
        print("ERROR: ARKEOLOGY_IMPORT.yaml not found in the current directory.")
        return 1
    ...
    if errors:
        print(f"ARKEOLOGY_IMPORT.yaml validation FAILED — {len(errors)} issue(s):\n")
        for err in errors:
            print(f"  - {err}")
        return 1

    print(f"ARKEOLOGY_IMPORT.yaml OK — {pending} pending entries ready to migrate.")
    return 0
