...
description: Content-scan own-scope cairn artifacts for unresolved frontmatter reference paths, resolve them against the migration path→artifact_id map, and backfill confirmed matches into the structured references field via link_metadata after a single dry-run batch report. Optional, decoupled, post-hoc cleanup for references that could not be resolved at migration time — declining leaves every artifact unchanged.
---
...
`migrating-to-cairn`, or periodically as new artifacts are written.
...
Before starting, read the project's `AGENTS.md` `<!-- cairn-mcp:config` block (team/project
...
- **Own-scope only.** This skill discovers and backfills only the deployment's own artifacts —
  never foreign-scope ones, consistent with every other reverse-lookup in cairn-mcp.
...
2. **Pre-flight — scope and manifest** — read the project `AGENTS.md` for the
   `<!-- cairn-mcp:config` block. If absent, stop: run `setting-up-cairn` first. Parse `team`
...
3. **Build the path→full-key map**
   ...
   **Determine `write_prefix` first.** Call `list_artifacts(team=<team>, project=<project>,
   ...
   - **No manifest found** — reconstruct an equivalent entry list by re-running the
     `migrating-to-cairn` skill's Step 2 discovery scan and Step 2c/Step 3 classification rules
