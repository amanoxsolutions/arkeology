...
links. Once a file becomes an Arkeology artifact, its address is a deterministic `artifact_id`
...
- "D3 — Content rewrite target format: resolved references are rewritten in stored content to the existing `arkeology://artifact/{id}` MCP resource URI. Unresolved or excluded targets keep their original raw path text untouched — mixed addressing across the corpus (`arkeology://...` next to `/docs/...`) is the correct permanent steady state, not a defect."
- "D4 — Forward-reference resolution: migration builds a single authoritative path→artifact_id map from the FULL `ARKEOLOGY_IMPORT.yaml` manifest (every entry, any status, across sessions) as soon as each entry's title+date are known and before any writes happen — since `generate_artifact_id` is a pure function, this resolves same-batch and multi-session forward references with no dependency on write order."
- "D5 — `http://`/`https://` entries in `references:` are always left untouched, never treated as path-resolution candidates."
- "D6 — Path normalization ceiling (best-effort, explicitly bounded): convert backslashes to forward slashes; try the lookup stripping common leading-prefix variants (`./`, a single leading `/`, or none). No match → leave completely untouched and fall through to the general search-based fallback. Repairing genuinely broken/inconsistent source references beyond this is explicitly not Arkeology's job."
...
- "Sweep — the proposed one-time re-link of `commit_refs` backfilled under the vector-only `link_commit`, so they land in annotations. Closed as not applicable: Arkeology has no live deployment carrying vector-only `commit_refs`, so there is nothing to sweep, and the union-of-both authority model means a value present in only one store self-heals rather than being dropped. Recorded in `docs/architecture-decisions/adr-2026-07-03-annotation-backed-link-storage.md` under its Consequences section. Would become relevant only if annotations were adopted on a deployment that had already accumulated vector-only `commit_refs`."
- "Cross-team reference leakage via a human-readable artifact_id appearing in another team's shared artifact — raised, examined, and explicitly closed as not applicable. A `references` entry can only ever hold a resolved artifact_id, which means the target is already a real Arkeology artifact; no special validation or restriction is being added for the cross-scope case."
...
- "Rewriting cross-references requires new content-mutation machinery → reframed: Arkeology already ships a resolvable URI scheme (`arkeology://artifact/{id}`) and a generic list-membership query operator (`$eq`) — the design reuses both instead of inventing new plumbing."
...
- "Cross-team reference leakage via a human-readable artifact_id needs new validation → challenged, then explicitly closed as not applicable, since a `references` entry can only exist as a resolved artifact_id if the target is already a real Arkeology artifact."
...
The existing migration capability (`skills/migrating-to-arkeology/SKILL.md`, backed by the
...
this is a genuine gap: `src/arkeology/artifact.py` has no `references` field today (the
...
- The existing `arkeology://artifact/{id}` MCP resource URI, already registered in `resources.py`
...
3. **Target format** (C) — plain id vs. `arkeology://` URI vs. promoted schema field
4. **Failure handling** (E) — what happens when the target isn't in Arkeology at all
...
cross-referencing gap** (Arkeology has no first-class way for any artifact to reference
another), not a migration-only concern. Four candidate directions were proposed against that
lens (first-class schema field; reusing the existing `arkeology://` URI via a general resolver;
...
- **`arkeology://artifact/{id}`** (D3) as the content-embedded target format — an existing,
...
artifact_id, meaning the target is already a real Arkeology artifact reachable by *some* existing
...
- ⚠️ **Overwrite wipes annotations** — overwriting an object *replaces* its annotations;
  Arkeology's tier-3 "updated in place" re-writes would silently drop accumulated link-data
  unless re-copied. Direct interaction with an Arkeology invariant.
...
validation is placed in the `setting-up-arkeology` skill plus graceful runtime handling, NOT a
