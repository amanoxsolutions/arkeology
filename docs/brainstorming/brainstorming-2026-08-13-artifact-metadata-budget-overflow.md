...
- D6 (OQ6 resolved) — API surface fate: `references=` filter parameters are removed outright from `list_artifacts` (breaking change, documented before shipped) since the field is no longer vector-filterable; `search_artifacts` has never had a `references` parameter, so it is unaffected. `commit_refs=` filter parameters are **kept unchanged** — the field remains filterable, per D2. The delete/archive reverse-lookup warning keeps scanning `source_artifacts` only; the `references`-half is dropped with no fallback, and the gap is documented rather than silently absorbed.
...
- **Direction 2 (Cluster 3 — Capacity/representation, final).** Remove `commit_refs`/`references`
  ...
  `list_artifacts` filtering by `references=[...]`/`commit_refs=[...]` (`search_artifacts` has
  never had these filter parameters), and
