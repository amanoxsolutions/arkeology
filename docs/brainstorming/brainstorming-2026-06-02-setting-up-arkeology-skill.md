...
Should the Arkeology installation and provisioning workflow — currently ~400 lines in the
README (clone, Python setup, AWS provisioning, IAM policy, .env, MCP client config, AGENTS.md
snippet) — be moved into a `setting-up-arkeology` skill, mirroring what was done for the
`migrating-to-arkeology` skill? This session explores the design space: whether the skill makes
...
|-----------|---------|------------|
| `<gerund>-arkeology` (e.g. install variant) | `<gerund>-<noun>` | Matches the migration skill pattern exactly (`migrating-to-arkeology`). Clear, discoverable. |
| `setting-up-arkeology` | `<gerund>-<preposition>-<noun>` | Accurate but longer; "setting up" is vaguer than "installing". |
| `provisioning-arkeology` | `<gerund>-<noun>` | Accurate for the AWS steps only — undersells the full scope (clone, config, MCP client). |
| `onboarding-to-arkeology` | `<gerund>-<preposition>-<noun>` | Too broad; "onboarding" implies team workflow, not server setup. |

**Verdict:** `setting-up-arkeology` is the chosen name.
...
AGENTS.md editing. However, all of these are in service of one goal: getting Arkeology running for
...
**Step 8 — Smoke test**
Start the server (`uv run arkeology`) and call `health_check`. If any component shows `"status":
...
The migration skill runs against an Arkeology instance that is already healthy — `health_check`
is Step 1. The installation skill builds the instance from scratch, so **no Arkeology MCP tools
...
the skill can use the same technique as `migrating-to-arkeology` (one `##` section per step) and should
...
1. **Build the `setting-up-arkeology` skill** — the fit is good, the agent advantage is real, the
...
3. **Move AGENTS.md snippet to `references/`** — the snippet is 140 lines. Embedding it inline
   ...
   `skills/setting-up-arkeology/references/agents-snippet.md` and instruct the agent to load it
...
The ADR strategy decision (git-only vs arkeology-only) currently lives in the migration skill's
...
5. **Positive-permission model** — list what IS permitted in Arkeology; everything else stays
...
8. **Structured HTML-comment config block** — a `<!-- arkeology:config ... -->` block
...
12. **Check AGENTS.md for config block** — migration skill looks for `<!-- arkeology:config`
...
file the operator wants permanently excluded from Arkeology.
...
#### D5 — Structured `<!-- arkeology:config ... -->` block in AGENTS.md
...
1. Check AGENTS.md for `<!-- arkeology:config`.
2. If absent → stop: "Arkeology does not appear to be installed for this project.
   Run the `setting-up-arkeology` skill first, then return here."
...
**Part A — ADR strategy:**
Ask the operator: git-only or arkeology-only. If git-only:
...
Ask: "Are there any other folders or files that should never go into Arkeology?
...
Write the `<!-- arkeology:config ... -->` block to AGENTS.md. If a block already exists
...
> "Before writing any artifact, check the `arkeology:config` block in AGENTS.md.
...
identified as scope creep: Arkeology is an application server, not an infrastructure
provisioning tool. Operators who adopt Arkeology already have AWS workflows and tooling
