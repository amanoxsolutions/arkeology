...
revised:
  ...
  date:
techniques_used:
  - inversion ("what would make OKF adoption a mistake for cairn?")
  - perspective-shift (cairn as a service vs OKF as a file format)
...
# OKF (Open Knowledge Format) Alignment for cairn-mcp
...
for progressive disclosure. This session explores what OKF means for cairn-mcp — a service that
...
OKF standardizes almost exactly the kind of artifact cairn **stores** (typed markdown knowledge
artifacts — produced by agents and humans, not by cairn itself), while deliberately omitting
everything cairn provides around them (durable storage, semantic search, access control). Is OKF a
threat, an opportunity, or both — and how far, if at all, should cairn lean into it?

### Known Constraints

| Constraint | Source |
|---|---|
| cairn artifacts are markdown `content` + structured metadata (`type`, `team`, `project`, `tier`, `date`, `status`, `title`, `description`, `visibility`, `feature_tags`, `author_role`, `source_artifacts`, `commit_refs`) | `artifact.py` |
...
### OKF ↔ cairn field mapping (reference)

| OKF frontmatter | cairn `Artifact` field |
|---|---|
...
| *(none)* | `tier`, `visibility`, `team`, `project`, `author_role` (cairn-only; ride along as custom keys) |

### Ideas Explored

**A. Export / interchange.** `cairn export --okf <scope>` → conformant bundle (git repo + `index.md`
...
**B. Import / ingestion.** `migrating-to-cairn` accepts an OKF bundle as an input source; an
`import_okf` tool ingests foreign bundles (e.g. Google's BigQuery enrichment output) and embeds each
concept; free-form OKF `type` mapped onto cairn's enum with an `okf:unknown` fallback.

**C. Native schema alignment.** Rename/add fields (`tags`↔`feature_tags`, `timestamp`↔`date`, add
`resource`) for lossless round-trip; track `okf_version`; carry cairn-only fields as OKF-legal custom
keys.

**D. Strategic positioning.** Position cairn as the durable, searchable, access-controlled backend for
the format Google standardized — the service layer OKF omits. OKF validates cairn's tier-2 artifact-gap
thesis. Treat Google's free bundle (Knowledge Catalog + enrichment agent + visualizer) as a competitive
signal and name cairn's differentiated lane.
...
- **Adapter layer** (A, B): export + import as adapters; cairn schema unchanged.
- **Native alignment** (C): bend cairn's model toward OKF.
- **Strategic positioning** (D): how cairn relates to Google's stack.
...
"good-enough, zero-infra" substitute for cairn's entire backend.

**Counter-thesis (cairn's moat):** progressive-disclosure / structural navigation degrades in recall
...
cairn stores `content` **verbatim** (`write.py:262`) — it never adds, parses, validates, or
transforms any YAML frontmatter inside it. Whatever frontmatter a document carries is whatever the
**authoring agent/human** wrote (OKF-shaped, amanox-template-shaped, or none). Separately, cairn keeps
two metadata structures of its own:

| Layer | What it is | cairn's role re: OKF |
|---|---|---|
| **1. Document frontmatter** | A YAML block inside the stored `content`. **Authored by the agent/human, not cairn.** | **Out of scope.** Making a document OKF-compliant is the *writer's* responsibility. |
| **2. S3 object metadata** (`s3_metadata`, `write.py:234`) | String key-values cairn attaches to the S3 object. | Not a document; a *source* for best-effort enrichment on export. |
...
**Conclusion (corrected in session — scope boundary):** cairn does **not** convert non-OKF documents
into OKF format. That is explicitly **out of scope** and is the authoring agent's responsibility. If
cairn ever offers an OKF *export*, it is **best-effort**: take the document's existing frontmatter,
adapt it, fill gaps from cairn's own stored metadata where possible, and add cairn's extra tags — with
no guarantee of OKF compliance. The vector index (layer 3) stays OKF-agnostic regardless (D5). This
reframes D2 (see below).

### Selected Directions

- **D1 — OKF as an export/interchange adapter (not a native rewrite).** `cairn export --okf`; optional
  import. cairn's S3/Vectors/Bedrock service stays authoritative; OKF is a portability skin. Also
...
- **D2 — Frontmatter handling, decided field-by-field. (LOCKED → boundary mapping + one internal rename.)**
  cairn maps to OKF at the **export boundary** as best-effort enrichment of the document's existing
  ...
  - **`date`: keep internal; map at the boundary — do NOT rename to `timestamp`.** cairn `date` is a
    ...
    *last-modified datetime* — a different concept. Honest export mapping: OKF `timestamp` ← derived
    from `last_edited_ulid` (cairn's real last-modified); cairn `date` has no native OKF slot → rides as
    a custom key.
  ...
  - **D2a — Best-effort OKF enrichment on export (layer 1).** On export, preserve the document's
    existing agent-authored frontmatter and *enrich* it from cairn's stored metadata where possible
    (`tags`; `timestamp` ← `last_edited_ulid`; `date` → custom key; add `resource`; carry cairn-only
    fields as custom keys). Best-effort, **not** authoritative — cairn never guarantees OKF compliance
    and never transforms non-OKF content. Rides with the exporter (D1, deferred).
    *Open detail for D1: precedence when existing frontmatter and cairn metadata disagree — default
    proposal: existing frontmatter is authoritative, cairn only fills gaps and appends its extra tags.*
  ...
  **Fork raised in session — does aligning internal names to OKF *simplify* things?** A real kernel:
  ...
  hides a *semantic* mismatch (cairn `date` is date-only vs OKF `timestamp` datetime; cairn `type` is a
  ...
  - **Option A — adopt OKF vocabulary as cairn's canonical names.** Max simplicity now (greenfield, no
    migration); couples the core model to a v0.1 draft and inherits the semantic mismatches.
  - **Option B — keep cairn vocabulary, align only at the boundary (= D2a mapping).** Captures the
  ...
  (`status`/`references`/`authored`/`techniques_used`/`decisions_*`) — neither cairn's metadata nor
...
- **D6 — OKF compatibility is a first-class product commitment. (LOCKED — resolves OQ3.)** cairn commits
  to OKF interoperability as a product pillar, not a one-off convenience: (a) advertise cairn as
...
- **D3 — Strategic positioning: "semantic memory" vs "progressive-disclosure wiki." (LOCKED.)** Update
  the project README (and `docs/architecture-decisions/overview.md` if appropriate) to position cairn as
