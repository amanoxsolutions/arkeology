...
Arkeology supports `adr` as an artifact type, yet the migration skill instructs operators to
...
what role does Arkeology play — authoritative store, search index, draft staging area, or
...
by the same skill instructs future agents to write `adr` artifacts to Arkeology without any
...
The cleanup table implies Arkeology is an index (the git copy is canonical, don't delete it).
The AGENTS.md snippet implies Arkeology is the destination (agents write ADRs there). Neither
piece of guidance says what happens to the git file when an agent writes a new ADR to
Arkeology. The gap is "who commits to git?"
...
#### Option 1 — Git is canonical, Arkeology is a search index

ADRs are authored by humans and committed to git. After a commit, the ADR is indexed in
Arkeology (either manually, via the migration skill, or by an agent that notices the gap).
Arkeology is read-only from the agent's perspective for ADRs; agents search and read, never
write new approved ADRs to Arkeology directly.

| Criterion | Score | Notes |
|---|---|---|
| Consistency | ⚠️ Medium | Indexing is a separate step; a committed ADR may be stale in Arkeology until re-indexed. An agent that reads the stale Arkeology copy acts on outdated information — this is a real failure mode, not just a cosmetic one. |
...
| Simplicity | ⚠️ Medium | The indexing step is not automatic. The workflow is: commit to git → also write to Arkeology. If the second step is forgotten, Arkeology is stale. |
| Reversibility | ✅ High | ADRs are fully intact in git if Arkeology is decommissioned. |
...
#### Option 2 — Arkeology is canonical, git is a mirror

Agents author ADRs via `write_artifact`. A commit hook or manual export step writes the
Arkeology content to `docs/adr/` in git. The git copy is human-readable backup.

| Criterion | Score | Notes |
|---|---|---|
| Consistency | ⚠️ Medium | Export step is required; without it git diverges from Arkeology. |
...
| Reversibility | ⚠️ Medium | Depends entirely on export discipline. If Arkeology is decommissioned without a recent export, ADR history lives only in the vector store, not in git. |
...
Agents write ADRs to Arkeology (for search) AND commit a markdown file to git (for history).
...
know which ADRs have drifted. `reconcile_index` can rebuild Arkeology from S3, but not from
git — there is no git-to-arkeology reconciliation path.

---

#### Option 4 — Arkeology for drafts, git for approved ADRs

ADRs go through a lifecycle: draft → approved. During the draft phase, an agent (or the
operator) writes the ADR to Arkeology as `visibility=hidden`. When the ADR is approved (by
the team, in a PR), two things happen: the final content is committed to git, and the
Arkeology artifact is updated to `visibility=shared`. Git is the approval signal.

| Criterion | Score | Notes |
|---|---|---|
| Consistency | ✅ High | The two copies are in sync by construction: the approved text is committed to git and simultaneously updated in Arkeology to shared. Divergence is bounded to the draft phase (hidden, so agents don't search it yet). |
...
| Simplicity | ⚠️ Medium | Requires understanding the two-phase lifecycle. The approval step requires two actions (git commit + Arkeology update). A small risk: the approval is committed to git but the Arkeology artifact remains hidden, so the approved ADR is invisible to agent search. |
| Reversibility | ✅ High | Approved ADRs are in git; Arkeology is the search layer. |

**Risk**: The approval step requires two deliberate actions (commit to git, update Arkeology
visibility). Neither action triggers the other automatically. If only one is done:
- Committed to git but not promoted in arkeology: the ADR is invisible to agent search (stale index, still hidden). Agents miss it.
- Promoted in Arkeology but not committed to git: the ADR is searchable but not in the repo. Reversibility is compromised.

This lifecycle is elegant but requires tooling or habit to enforce the two-action approval step.

---

#### Option 5 — No ADRs in Arkeology at all

Remove `adr` from the type catalogue. All ADRs stay in git. Semantic search over ADRs is not
supported by Arkeology. Teams use git grep, GitHub search, or a separate tool.
...
Arkeology (in the brainstorming-artifact-store.md, OQ5 and cross-team discovery) explicitly
...
|---|---|---|---|---|---|
| 1 — Git canonical, Arkeology index | ⚠️ | ✅ | ✅ | ⚠️ | ✅ |
| 2 — Arkeology canonical, git mirror | ⚠️ | ❌ | ✅ | ❌ | ⚠️ |
| 3 — Dual write, git wins | ❌ | ✅ | ✅ | ❌ | ✅ |
| 4 — Arkeology drafts, git approved | ✅ | ✅ | ✅ | ⚠️ | ✅ |
| 5 — No ADRs in Arkeology | ✅ | ✅ | ❌ | ✅ | ✅ |
...
Arkeology for searchability, keep git as the authority. The skill already handles the initial
bulk indexing. The only missing piece is explicit guidance on what to do when an ADR changes.

**The key additions needed:**

1. The migration skill's cleanup guidance should state explicitly that Arkeology holds an
   *index copy* of ADRs, not the authoritative copy.

2. The AGENTS.md snippet must distinguish between reading ADRs (via Arkeology search, then
   read) and writing ADRs (to Arkeology for indexing, after the human has committed to git).
   The current snippet instructs agents to write `adr` to Arkeology without this nuance —
   it will be read as "Arkeology is the destination", which is incorrect.

3. When an agent writes an ADR to Arkeology, it should note to the operator that the git copy
   must be committed separately. This is a convention, not a technical enforcement.

4. The stale-index risk (ADR updated in git, not re-indexed in Arkeology) should be
   acknowledged as a known limitation. The `reconcile_index` tool mitigates it partially —
   but reconcile rebuilds the vector index from S3 (Arkeology's own store), not from git.
   If an ADR was updated only in git and never re-indexed to Arkeology, reconcile will not
...
relationship to the Arkeology copy should be going forward).

**Proposed revised entry:**

| Type | Safe to remove from repo | Notes |
|---|---|---|
| `adr` | **No — keep in git permanently** | Arkeology holds a *search index copy*, not the authoritative version. Git is the source of truth. When an ADR is updated in git, re-index it in Arkeology by calling `write_artifact` again with the new content. Agents use Arkeology to *find* ADRs; the git file is the definitive text. |
...
This is ambiguous — it implies agents are the *authors* of ADRs in Arkeology, with no
...
index it in arkeology by calling `write_artifact` (tier 3, visibility=shared). When an ADR
is updated in git, call `write_artifact` again with the new content. To find relevant ADRs
mid-session, use `search_artifacts` — but always treat the arkeology copy as a search index:
if there is any doubt about currency, read the git file directly.
```

The AGENTS.md snippet should not say "write ADRs to Arkeology" without the accompanying
"after committing to git" qualification. Without that qualification, agents will treat Arkeology
...
is not in Arkeology. An agent searching for related decisions won't find it. This is the core
stale-index risk of Option 1. Mitigation: the AGENTS.md snippet should instruct agents to
check for new ADR files in `docs/adr/` and index any that are absent from Arkeology
...
by ADR-012." If the Arkeology copy is not re-indexed, agents will find and read the superseded
ADR without knowing it has been replaced. The `status` metadata field in Arkeology should be
...
the Arkeology artifact will remain unless explicitly deleted or archived. `archive_artifact`
...
an ADR in Arkeology (tier 3, hidden) for human review, the lifecycle is: hidden draft in
Arkeology → human reviews → human commits to git → agent updates Arkeology to shared.
...
Arkeology as `visibility=hidden` for review. Once committed to git, update Arkeology to
`visibility=shared`."

**5. Cross-team ADR discovery**

The original vision for tier 3 cross-team sharing depends on ADRs in Arkeology being
...
| **Discoverability without a running service** | ✅ High — an engineer debugging a mystery must be able to grep for relevant ADRs without needing AWS credentials or a running Arkeology instance. | ✅ Medium — vision.md/requirements.md and plan.md in `docs/` are easily browsed on GitHub. Useful but not critical; the spec is not usually opened mid-debug. | ⚠️ Low — plans are read in planning sessions, not mid-debug. | ❌ None — syntheses are AI-produced summaries; engineers do not browse them. |
...
### 2. What does Arkeology uniquely provide, and does it matter equally across tier 3 types?

| Arkeology property | `adr` | `spec` | `plan` | `synthesis` |
|---|---|---|---|---|
...
**Observation:** Arkeology's value is roughly equal for all four types on semantic search and
...
Plans and specs change continuously. This does matter: high-frequency writes to Arkeology are
cheap and natural. High-frequency writes to git require commit discipline, PR overhead, and
reviewer fatigue. *Change frequency favours Arkeology as the write destination for plans and
...
for ADRs.* This supports git as the natural home for ADRs and Arkeology as the natural home for
the others.
...
with this for X reason" and that objection is preserved in the PR thread. Arkeology has no
...
in Arkeology. This applies to `adr` by default. It applies to `spec` only if the team uses
...
**What happens when Arkeology is decommissioned:** All tier 3 artifacts that were never
...
without formal approval can live in Arkeology as canonical. Both types can be indexed in
Arkeology for search.

**What this implies concretely:**

- `adr` → PR-approved by convention in most teams → git is canonical, Arkeology is the index.
- `spec` (approved) → If the team uses PR review for spec approval → git is canonical, Arkeology is the index. Same treatment as ADRs.
- `spec` (working draft) → Agent-authored, no formal approval yet → Arkeology is canonical, may or may not be committed to git.
...
**The grey zone:** A spec that starts as a working draft in Arkeology (`visibility=hidden`) and
then gets formally approved via PR — what happens? The PR-merged version in git diverges from
the Arkeology draft. Someone must reconcile. Position C does not specify the reconciliation
...
#### Position D — Arkeology and git serve completely different purposes; the question of "canonical" is a false dichotomy
...
RFCs). Arkeology is for **agent knowledge** (what agents learned, reasoned about, produced, or
...
- An ADR committed to git is the code-governance record. It is owned by the engineering team's
  code review process.
- The same ADR in Arkeology is the agent-knowledge record. It is owned by the agent runtime.
- Neither copy "wins." They serve different consumers (human engineers vs. AI agents) and
  different tooling (git/GitHub vs. MCP/arkeology).
...
Arkeology copy of an ADR is a problem only for agents that depend on it being current. A stale
git copy would be bizarre (git copies don't go stale; commits are append-only). The real risk
is an Arkeology copy that is never updated after the git ADR is superseded. But this is a
...
git copy is the authority for that approval, while Arkeology is the authority for agent search."

---

### Synthesised view — which position best fits Arkeology's design intent?
...
that do not can be arkeology-canonical).
...
> without formal approval can live in Arkeology as the primary store, optionally mirrored to
...
- `plan` does not typically require formal approval, changes continuously, and serves primarily
  ...
  human browsability.
- `synthesis` is never PR-approved and serves exclusively agent consumers → Arkeology is the
...
a PR, and both are indexed in Arkeology for search.* The ADR vs. spec distinction is a
...
3. **Does the `spec` type in Arkeology conflate two distinct sub-types** — "approved spec" (which
