...
| **Discoverability without a running service** | ✅ High — an engineer debugging a mystery must be able to grep for relevant ADRs without needing AWS credentials or a running Arkeology instance. | ✅ Medium — the PRD and plan in `docs/` are easily browsed on GitHub. Useful but not critical; the spec is not usually opened mid-debug. | ⚠️ Low — plans are read in planning sessions, not mid-debug. | ❌ None — syntheses are AI-produced summaries; engineers do not browse them. |
