...
# Design + accessibility review of Arkeology studio (p11-t44 browser UI)
...
reference loads **Open Sans from the Google Fonts CDN; Arkeology must not** — pointing at
...
- The shipped `arkeology-studio.html` uses real `<select>`, `<form role="search">`, and `aria-label`s.
...
1. **[✅ Resolved 2026-06-29] GDPR: the spec and CSP mandate Google Fonts; this must be removed.**
   ...
   - This is exactly the dependency you've ruled out. **It is also already contradicted by the
     implementation:** `arkeology-studio.html:340` uses `system-ui, -apple-system, …` and loads **no**
   ...
   - **Fix:** drop Inter/Google-Fonts from T44 (keep the `system-ui` stack the design HTML and
     impl already use, or self-host a font under `src/arkeology/static/` via `@font-face` — OFL
...
2. **[✅ Resolved 2026-06-29] `--t3` (#404870) fails WCAG AA badly (~2.2:1) yet the spec routes readable text through it.**
   ...
   - Despite that, the **normative spec** sends real content to `--t3`: list-item "date + tier in
     ...
     shipped app follows the spec — `arkeology-studio.html` uses `--t3` for `.li-meta` (L715/719) and
...
4. **[✅ Resolved 2026-06-29] `.li:not(.active){ opacity:.55 }` dims every unselected row (design HTML L240).** In a list,
   ...
   `.li.active`). (The shipped `arkeology-studio.html` appears **not** to replicate this rule — good;
...
5. **[✅ Resolved 2026-06-29] No visible focus indicators; keyboard users are stranded.** The design HTML has no
   ...
   `#filters select` use `outline: none` (arkeology-studio.html L416/476) with only a faint
...
6. **List semantics are broken for assistive tech.** `arkeology-studio.html` sets
...
11. **Dark-only; the host may be light.** The benchmark ships a designed light theme via
    `[data-theme="light"]`. Arkeology is dark-only, so the iframe stays dark inside a light
...
- 2026-06-29 — **C1 (Google Fonts) fixed & verified.** `resources.py` `ResourceCSP`
  ...
  specs updated (origins, Typography, TL;DR, Boundaries); `arkeology-studio.html` and the design
...
- 2026-06-29 — **m11 (light theme + toggle).** Designed `[data-theme="light"]` token overrides
  ...
  first load — in both `arkeology-studio.html` and the design companion. Mermaid theme follows the
  active theme.
