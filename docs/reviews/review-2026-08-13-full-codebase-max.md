...
**[2026-08-20 — ✅ RESOLVED.** The original same-day resolution note above was itself incorrect and has been corrected by a follow-up verification review (same day). It claimed `purge.py` and `propose_commit_links.py` were "already fully wrapped" as T63/T68 byproducts — neither actually was; the claim was never checked against real file content (`purge.py` had zero diff across the entire post-review commit range). It also undercounted `write.py`'s and `archive.py`'s remaining raw call sites: the T69 fix wrapped only literal `s3.*`/`vectors.*` call sites, missing calls to helper functions one level removed — `fetch_vectors_by_metadata`/`find_referrers` (in `_search_helper.py`) and `read_current_link_fields`/`apply_link_annotations` (in `annotations.py`). `migrate_artifacts.py`'s existence-check loop and `delete.py` were correctly wrapped all along — confirmed, not part of the gap. The actual gap is now fixed: all 11 call sites across the 4 affected files route through `asyncio.to_thread`: `purge.py`'s `_purge_archived_inner` — 2 sites (`fetch_vectors_by_metadata` for inactive-artifact discovery and active-synthesis discovery); `propose_commit_links.py`'s candidate-discovery step — 1 site (`fetch_vectors_by_metadata`); `write.py`'s overwrite CAS retry loop + fresh-create branch — 3 sites (`read_current_link_fields`, `apply_link_annotations` ×2); `archive.py` — 5 sites (`fetch_vectors_by_metadata` for the idempotency check, `find_referrers` for the referenced-by check, `read_current_link_fields` + `apply_link_annotations` for the CAS status re-PUT loop, `fetch_vectors_by_metadata` with `include_data=True` for the vector-flip loop). `link_metadata.py`'s two `head_object` calls inside `_apply_link_metadata_with_cas` remain deliberately untouched: that helper is a plain (non-`async`) `def` already invoked as a single unit via `await asyncio.to_thread(_apply_link_metadata_with_cas, ...)` at its one call site, so wrapping the inner calls individually is both impossible (no `await` in a sync `def`) and redundant. `health.py`'s pre-existing, documented exception is unchanged. Verified: full unit suite passes (1277 passed, same pass set — pure refactor, no behavioural change); ruff check, ruff format --check, and mypy clean. Phase 12 T69, `plan.md` now `✅`.]
