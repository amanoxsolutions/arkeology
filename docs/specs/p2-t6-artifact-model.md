...
- The valid artifact types are: `code_review`, `session_summary`, `implementation_note`,
  `spec`, `adr`, `bug_report`, `decision_note`, `synthesis`, `brainstorming`. This set is the
  ...
  `postmortem`, and later again with `learning` — see those specs for the current full set.)
