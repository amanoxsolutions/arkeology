...
Connected agents currently have no runtime mechanism to discover the cairn-mcp schema.
...
An agent starting a new session calls `cairn://schema/artifact` and knows immediately
...
**Acceptance criteria:**
- Given a connected agent reads `cairn://schema/tiers`, then the response describes:
...
**Acceptance criteria:**
- Given a connected agent reads `cairn://schema/visibility`, then the response describes
...
**Acceptance criteria:**
- Given a connected agent reads `cairn://schema/types`, then the response lists every type
...
- Given a connected agent calls `resources/list`, then all five URIs are present:
  `cairn://schema/artifact`, `cairn://schema/tiers`, `cairn://schema/visibility`,
  `cairn://schema/types`, `cairn://schema/query-strategy`.

## Requirements

- THE SYSTEM SHALL expose five MCP Resources at these URIs: `cairn://schema/artifact`,
  `cairn://schema/tiers`, `cairn://schema/visibility`, `cairn://schema/types`,
...
- THE `cairn://schema/artifact` resource SHALL derive its list of valid types from
  `ARTIFACT_TYPES` in `artifact.py` — not a hardcoded copy.
- THE `cairn://schema/types` resource SHALL also derive its type list from `ARTIFACT_TYPES`
  directly so both stay in sync automatically on schema changes.
- THE SYSTEM SHALL implement resources in a dedicated `src/cairn_mcp/resources.py` module
...
| `src/cairn_mcp/resources.py` | Create | Written after unit tests (Green) |
| `src/cairn_mcp/server.py` | Modify | Call `register_resources(_app)` — add the import and call |
...
- If `ARTIFACT_TYPES` is temporarily monkey-patched to add a fake type during the test,
  both `cairn://schema/artifact` and `cairn://schema/types` responses include the new
