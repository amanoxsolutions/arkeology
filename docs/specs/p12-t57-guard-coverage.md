...
### Story 3 — `link_metadata` rejects a control character in a supplied `commit_refs`/`references` element (P1)
...
- Delegate the control-character and comma checks to `Artifact.validate_commit_refs`/`validate_references` directly (verified viable — see Requirements) rather than reimplementing `_require_no_control_chars`/`_require_no_comma` a second time. This closes the exact drift risk of a hand-rolled parallel implementation: a future change to either `_require_*` helper (e.g. a new forbidden character class) would otherwise need to be manually mirrored into `link_metadata.py` to stay correct, and the missing control-character check is proof that mirroring already failed once.
- Keep the existing empty/whitespace-only rejection in `_validate_supplied_link_values` — this is a `link_metadata`-specific constraint the `Artifact` model does not itself enforce (an element `Artifact` would silently accept); only the missing control-character check needs to be added, not removed.

**Ask First:**
- Nothing — placement and behaviour are fixed by ADR D1; the control-character delegation shape (call the classmethods directly, verified callable outside model construction) has no open design choice either.
...
3. **`test_tools_link_metadata.py` → `link_metadata.py`** (control-character check) — call `link_metadata` with a supplied `commit_refs` element containing a Unicode `Cc` control character (e.g. `"abc\x01def"`); assert `validation_error`, zero `put_object_annotation` calls, zero `put_vectors_batch` calls (`mocker.spy` on both). Repeat for `references`. Assert a supplied element with no control character, no comma, and no empty/whitespace-only content still succeeds unchanged (regression guard for the delegation change).

Use `aws_mock`, `s3_client`, `vectors_client_*` fixtures; `FakeBedrockClient` for embeddings.

## Open Questions

*(none — placement, error mapping, and scope are fixed by ADR D1; this task is a pure prerequisite for T58. The control-character delegation shape (direct classmethod call) was verified viable against the current codebase during spec-writing — see Boundaries — so it carries no open design question either.)*
