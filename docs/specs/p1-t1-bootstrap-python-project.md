...
Before any cairn-mcp code can be written, the repository must be a properly structured, installable
...
- Given a installed project, when the developer runs `uv run cairn-mcp`, then the process starts without error (even if it immediately exits because no tools are registered yet).
...
- WHEN the developer runs `uv run cairn-mcp` THE SYSTEM SHALL start the process using the entry point defined in `pyproject.toml`.
...
- Source layout must follow `src/` layout: package lives at `src/cairn_mcp/`.
...
| `src/cairn_mcp/__init__.py` | Create | Package marker — empty or version string only |
| `src/cairn_mcp/__main__.py` | Create | Thin entry point stub — imports and calls server run (stub only at this stage) |
...
**Project metadata:** name `cairn-mcp`, version `0.1.0`, description, `requires-python = ">=3.12"`.
...
**Entry point:** `[project.scripts]` → `cairn-mcp = "cairn_mcp.__main__:main"`. This is what makes
`uv run cairn-mcp` work.
...
The point of wiring this up in T1 is purely to verify the entry point declaration works — `uv run cairn-mcp` must resolve and call `main()` without import errors.
...
- `uv run cairn-mcp` → exits 0 (stub), prints a line to stdout
