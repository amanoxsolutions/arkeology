...
_Status: **in progress — Phase 2 complete; Phase 3 implementation complete (T10–T16 done, 370 unit tests passing); Risks Q2–Q4 resolved via AWS docs research; T17 added as top-priority Phase 4 phase gate**_
...
- **S3 Vectors `PutVector` upsert behaviour** — documentation provides strong evidence for upsert semantics (key uniquely identifies the vector in the index; no `ConflictException` in PutVectors error responses; S3 Vectors overview uses "write, update, delete" language; no separate UpdateVector API exists); empirical confirmation required in T17; if integration test reveals non-upsert behaviour, tier 3 overwrite must be updated to explicitly delete existing section vectors before re-writing (code change + test update captured in T17 done condition)
...
- **S3 Vectors `PutVector` upsert — strong doc evidence, empirical confirmation pending**: documentation strongly implies upsert (overwrite) semantics when re-using an existing key — key "uniquely identifies the vector in a vector index", no `ConflictException` in PutVectors error responses, S3 Vectors overview describes "write, update, and delete" as first-class operations, no separate UpdateVector API exists; empirical confirmation in T17 integration test run
