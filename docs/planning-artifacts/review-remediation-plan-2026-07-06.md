...
- **Verification: complete.** **Fixes: not started (deferred by operator on 2026-07-06).**
- A dedicated branch **`review-followup-2026-07-06`** was cut off `main` for this work; it currently
  carries no code commits (the one attempted fix — minor #38 — was found not applicable, see below).
...
|---------|----------|-----|
| **M1** | `tools/write.py` annotation block; `clients/s3.py::put_object_annotation` re-raises non-annotation-unavailable | A transient/non-credential annotation `ClientError` (SlowDown/timeout) escapes to the blanket handler → orphaned S3 object, zero vectors, **no failure-log entry**. Degrade unknown annotation exceptions to warn+continue, or append a failure-log entry (M-4 fixed only the credential branch). |
| **M2** | `annotations.py::encode/decode_link_list`; `artifact.py::validate_commit_refs/validate_references` | Comma in a link-list element diverges the two stores (comma-join encoding). Reject `,` in elements, or switch the annotation payload to a JSON array. |
| **M9** | `tools/link_metadata.py`; `annotations.py::_read_vector_link_fields` | No value validation (empty/whitespace/comma pass; `[""]` diverges stores) — strip/reject. Annotation-read samples only `keys[0]`; union across all section vectors. (items[0] *write*-collapse already fixed.) |
| **M10** | `clients/s3.py::put_object_annotation`; `tools/link_metadata.py` loop | `put_object_annotation` lacks the `NoSuchKey→KeyError` map its siblings have → orphaned-vector artifact aborts the whole batch as `internal_error`, all accounting lost. Map + skip-and-count. |
| **M13** | `references.py::build_path_to_id_map` | Keys the map on raw `entry["path"]` (no `normalize_reference_path`) → a `./`- or backslash-spelled manifest path matches nothing → migration rewrite silent no-op. Normalize map keys. |

### FC-2 — Search / write validation correctness  *(developer; small, high-confidence)*
| Finding | Location | Fix |
|---------|----------|-----|
| 07-02 **#3** | `tools/write.py` / `write_artifacts.py`; `generate_artifact_id::_require_valid_tier` | String `tier:"2"` → uncaught `ValueError` → `internal_error`. Return `validation_error`. |
| 07-02 **#4** | `search.py`, `synthesise.py`, `server.py` params | `top_k <= 0` unvalidated → negative slices. Add `ge=1` guard. |
| 07-02 **#5** | `_search_helper.build_user_filters`, `list.py` | Filter enum values (type/status/tier) unvalidated → typos silently return empty. Validate against `ARTIFACT_TYPES`/status/tier enums → `validation_error`. |
| 07-02 **#7** | `search.py` (`SEARCH_FETCH_TOP_K` × `SEARCH_MAX_ITERATIONS` = 75) | Advertised `top_k=100` unreachable at defaults, no truncation signal. Signal when the loop exhausts below the requested count, or correct the docstring. |
| 07-02 **#13** | `search.py`, `synthesise.py` (`int(meta["tier"])`) | Hard `KeyError` on a vector missing `tier` while `list.py` is defensive (`meta.get("tier", 0)`). Make search/synthesise defensive. |
| 07-02 **#2** | `write.py` vector key `{s3_key}#{section_slug}`; `_section_pipeline.py` | Duplicate H2 headings collapse to one vector key → `sections_indexed` under-reports. Disambiguate duplicate slugs or count pre-dedup. |

### FC-3 — Client / config / robustness minors  *(developer)*
- **07-02:** #8 (`WRITE_PREFIX` validation weaker than `READ_PREFIXES`, `config.py`), #9 (fixed startup probe key → concurrent-server race, `startup.py`), #10 (check 3 proves only `ListBucket`), #11 (check 1 conflates missing bucket vs bad creds), #14 (`coerce_list_field` used in delete but not purge/freshness), #15 (`zero_results` asymmetry; synthesise silently drops failed reads), #17 (`get_vectors` `returnData=True` wastes float arrays for metadata-only reads; freshness 1+N), #18 (300-thread executor at import, `write.py`), #19 (failure log: no lock/rotation, cwd-relative default path), #20 (percent-decode mangles legacy `%XX`), #21 (`_URL_PREFIXES` case-sensitive, `references.py`), filter.py #20 (`filter` shadows builtin) / #21 (unsupported op raises mid-pagination) / #22 (mixed-type comparison `TypeError`), resources.py #31 (newline in description corrupts `cairn://artifacts` table) / #32 (typo "cairn studio— visual"), #37 (raw boto messages — bucket/index names — leak to MCP callers).
- **Phase-12 robustness:** #15 (`decode_link_list` keeps interior empty segments), #16 (non-UTF-8 annotation → unclassified `UnicodeDecodeError`), #17 (`link_metadata` `annotation_unavailable` discards partial `linked`/`skipped` counts), #18 (`apply_link_annotations` issues 2 pointless deletes per fresh write + warns on annotation-less deployments), #19 (`json.dumps` defaults over-count budget — use `ensure_ascii=False, separators=(",",":")`), #23 (duplicate manifest paths last-win silently), #27 (`find_referrers` = 2 full-index scans per delete/archive — merge to one; ties to #17).
- **#12** (informational): `SEARCH_FETCH_TOP_K le=100` — the real QueryVectors topK cap is 10 000 (confirmed by `test_top_k_over_documented_cap_is_rejected`); just annotate the bound's provenance.

### FC-4 — Studio UI hardening  *(developer; part enhancement — `static/cairn-studio.html`)*
- #23 (`tier` unescaped in list rows — `renderList`), #24 (prototype-chain type-map lookups → `Object.create(null)`/`Map`), #25 (pin `marked`/`mermaid` CDN versions like the SDK/DOMPurify), #26 (stale-response race — guard `selectArtifact` with `if (selectedId !== id) return`), #27 (`doSearch` ignores active facet filters — forward `filterType/Tier/Status`), #28 (no pagination for large stores), #29 (theme toggle re-downloads the artifact), #30 (`studio.py` fallback `structured_content` returns the whole unbounded listing).

### FC-5 — Missing tests  *(developer; some need real AWS)*
- **SA-3(a)** T47 integration: real-AWS write→backfill→overwrite→annotation-survives round-trip (`tests/integration/test_tools_write.py`).
- **SA-3(b)** T48 integration: backfill→drop vectors→`reconcile_index` restores both link fields (`tests/integration/test_tools_reconcile.py`).
- **M14** unit test for `list_object_annotations` `ContinuationToken` pagination (currently dead code under tests).
- 07-02 **#35** `nextToken` multi-page test for `list_vectors_by_metadata`.
- 07-02 **#36** assert `register_tools` registers the full tool set.
- Phase-12 **#26** remaining: list-on-missing-object; orphaned-vector-skip (ties to M10).
- 07-02 **#33** replace the 5 `set_credential_failure` call sites with `mocker.patch.object(..., side_effect=CredentialError)` and delete the deprecated method from `fake_bedrock.py`.
- Phase-12 **#24** add an autouse fixture to reset `_ANNOTATION_STORE` between tests (`conftest.py`).
- 07-02 **#34** replace the brittle `inspect.getsource` substring assertion (`test_tools_migrate_artifacts.py`).

### FC-6 — Docs / hygiene  *(tech-writer)*
- Phase-12 **#1** (brainstorming still lists OQ3 pending + retired sweep item), **#3** (`plan.md` "open — planning" vs "all implemented"; T55 "Spec: to be written" though the spec exists), **#4** (T53 Files-to-Touch describes the pre-consolidation symlink layout), **#5** (T54/T55 cite gitignored `.docs/` as provenance), **#7** (T55 lists `title`/`author_role` in both filterable and non-filterable), **#9** (stale `link_commit` comment in `tests/unit/test_tools_read.py`), **#10** (CHANGELOG `[Unreleased]` empty despite the breaking `link_commit`→`link_metadata` removal), **#12** ("S3 Express One Zone" double-counts "directory buckets" across README/SERVER-REFERENCE/ADR/credentials.py/PRD/brainstorming), **#14** (FR-51–58 what-not-how violations + FR-54 duplicates ADR-011 §1 → dual-maintenance), **#22** (skills reimplement `generate_artifact_id` twice with no drift test — add a drift test or single-source it), **#8** style residue ("D4" shorthand vs "decision 5").
- **M8** (straddles code+doc): the `setting-up-cairn` aws-cli probe path has no always-run cleanup, and the probe key isn't excluded by `reconcile.py`'s orphan scan (only `_cairn_health_probe`/`_cairn_mcp_startup_probe` are). Fix = reserved non-artifact prefix + reconcile exclusion (code) + always-run cleanup instruction (skill).
- **#13** (concept): mixed addressing (D3) has no documented consumer — `cairn://` links render dead in the studio app; specify read-surface treatment (or make the studio resolve `cairn://` — overlaps FC-4).

## Suggested order when resumed

1. Settle the **design decisions** (M3, M4, M5, CA-5) — architect memo → operator sign-off.
2. **FC-1** (link-storage integrity) — highest value; fold in the M4 outcome (references replace/removal).
3. **FC-2** (search/write validation).
4. **FC-5** (tests) — especially the two missing SA-3 integration round-trips (need a live-AWS run).
5. **FC-3** (robustness minors) — fold in M5 outcome (cross-scope reference filtering) and CA-5 (synthesise budget).
6. **FC-4** (studio) and **FC-6** (docs) — parallelisable, lower risk.
