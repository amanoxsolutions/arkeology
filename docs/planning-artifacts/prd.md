...
- Migration skill following the agentskills.io standard, shipped with the repository: structured agent-driven workflow to migrate existing project documentation into cairn-mcp; covers discovery, classification, metadata enrichment (Bedrock-generated descriptions, git date recovery), and post-migration guidance (AGENTS.md update, tier 2 file removal instructions); includes a bulk import script (bulk operations, dry-run preview support, configurable concurrent writes) and a structured manifest format; two paths: agent-only for very small projects (< 5 files, sequential) and manifest + script for 5+ files; for ≥ 10 files the agent-only path uses a two-phase approach (enrich all files first, then write in parallel batches of 4–5 via sub-agents)
...
| FR-23 | **Must** | The cairn-mcp repository ships a migration skill following the agentskills.io standard (https://agentskills.io/home). The skill provides a structured agent-driven workflow for migrating existing repo documentation into cairn-mcp and is intended for one-time use at adoption time. **Discovery**: the agent scans standard documentation directories (e.g. `docs/adr/`, `docs/specs/`, `docs/brainstorming/`, `docs/notes/`, `docs/planning-artifacts/`) and excludes non-artifact files (`README.md`, `AGENTS.md`, `CHANGELOG.md`, non-markdown files, auto-generated docs). **Classification**: the skill provides a directory-convention mapping table (e.g. `docs/adr/` → `adr` tier 3, `docs/specs/` → `spec` tier 3, `docs/brainstorming/` → `session-summary` tier 2) plus the agent's judgment for ambiguous files. **Metadata enrichment**: title extracted from H1 heading (fallback: filename); description generated by the agent in-context for ambiguous or high-value tier 3 files, or by the bulk import script via Bedrock for bulk clear-cut files; date recovered from git log (first commit for tier 2, last commit for tier 3), with frontmatter `date:` field and filename-embedded date as overrides and current date as fallback; `team` and `project` provided once by the operator and applied uniformly. **Two workflow paths**: agent-only (< 5 files — agent sweeps, classifies, generates descriptions, calls `write_artifact` for each sequentially) and manifest + script (≥ 5 files — agent produces a structured migration manifest, operator reviews, then the bulk import script executes concurrent bulk writes). For the agent-only path with ≥ 10 files, the agent uses a two-phase approach: complete all enrichment (read, describe, date-recover) for every file first, then split into batches of 4–5 and spawn sub-agents to write in parallel, collecting results before verification. **Post-migration**: the skill instructs the operator which tier 2 files may be removed from the repo; the skill's final step appends the cairn-mcp usage snippet to the project `AGENTS.md` so future agents write to cairn-mcp by default. The skill bundles a bulk import script: supports a dry-run preview mode that outputs structured JSON of what would be imported without writing; writes artifacts concurrently bounded by a configurable concurrency limit (default 3); outputs structured JSON to stdout; logs diagnostics to stderr; idempotent on re-run. A structured manifest format is documented with the skill. Installation is manual copy; the README documents the skills directory path for each supported IDE (Claude Code, Codex, Copilot, OpenCode). |
...
| NFR-15 | **Should** | **Migration throughput** — a bulk migration of 10 documents using the bulk import script at the default concurrency setting must complete in under 2 minutes under normal Bedrock response times. The migration skill's two-phase parallel-agent path for ≥ 10 files must reduce end-to-end wall-clock time (LLM enrichment + writing) to under 2 minutes for a 10-document migration. |
