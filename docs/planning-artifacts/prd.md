...
- MCP server with 8 tools and MCP Resources: write artifact, read artifact, search artifacts, list artifacts, archive artifact, health check, reconcile index, synthesise artifacts; plus schema discovery resources (artifact metadata schema, tier model, visibility model, type catalogue, query strategy guidance)
...
| FR-13 | **Must** | Tier 2 project-local artifacts are immutable once written. Content is frozen as a permanent, point-in-time record. No content editing capability is provided. |
| FR-14 | **Must** | Archiving an artifact is the only modification permitted on an existing tier 2 project-local artifact. It marks the artifact as inactive and excludes it from search and listing results by default. No other attribute — content, tier, visibility, title, or feature tags — can be modified after the initial write. |
