...
- **Commit-to-artifact traceability** — artifacts carry git commit SHA references, held durably as S3 object annotations and mirrored into the vector index as a filter, so a listing can answer "which artifacts came out of this commit". At the end of a session, unlinked artifacts can be discovered and linked to the session's commit SHA in a single confirmation step — without re-embedding. Commit refs can also be supplied at write time.
